...
class FWI_GFS_GSMAP:
    ...
    def __calculate_temperature(self):
        ...
        self.temp = self.gfs.select(temp_band).rename("T")
    ...
    def __calculate_relative_humidity(self):
        ...
        self.rhum = self.gfs.select(rhum_band).rename("H")
    ...
    def __calculate_rain(self):
        ...
        """
        self.rain = self.gsmap.reduce(ee.Reducer.sum()).rename("R")
    ...
    def __get_fwi_inputs(self):
        ...
        """
        local_noon = datetime(
            self.date.year, self.date.month, self.date.day, hour=12, tzinfo=dateutil.tz.gettz(self.timezone)
        )
    ...
    def preprocess(self, interpolation, crs, scale):
        ...
        """
        self.temp = self.temp.resample(interpolation).reproject(crs=crs, scale=scale)

        self.rhum = self.rhum.resample(interpolation).reproject(crs=crs, scale=scale)

        self.wind = self.wind.resample(interpolation).reproject(crs=crs, scale=scale)

        self.rain = self.rain.resample(interpolation).reproject(crs=crs, scale=scale)
...
class FWI_ERA5:
    ...
    def __calculate_relative_humidity(self):
        ...
        rhum = 100 * (
            math.exp(1) ** ((17.625 * dew) / (243.04 + dew)) / math.exp(1) ** ((17.625 * temp) / (243.04 + temp))
        )
    ...
    def __calculate_rain(self):
        ...
        """
        rain_24h = self.era5_rain.select("total_precipitation").reduce(ee.Reducer.sum())
    ...
    def __get_fwi_inputs(self):
        ...
        self.era5_rain = ee.ImageCollection("ECMWF/ERA5_LAND/HOURLY").filterDate(
            start_datetime.isoformat(), utc_datetime.isoformat()
        )
